...
在展开技术面/消息面之前，**必须先**写一小节「**标的信息**」或 **Quote snapshot**，数据须来自工具返回值（**禁止编造**）：
- **证券代码**：规范格式（A股如 `600519.SH`；港股 `HK00700` 或 `00700.HK`；美股如 `AAPL`）
- **股票名称**与**所属市场/交易所**
- **最新价**、**涨跌幅**、**成交量/成交额**（优先引用 `get_realtime_quote`；若该工具失败须说明原因）
- **数据时点**：报价时间或交易日

若用户使用英文提问，该小节用英文撰写（可用 **Symbol / Last / Chg% / Volume / As of** 等表头）。
